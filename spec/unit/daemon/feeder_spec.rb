...
describe Rapns::Daemon::Feeder do
  ...
  it "checks for new notifications with the ability to reconnect the database" do
    ...
    Rapns::Daemon::Feeder.deliver_notifications
  end
  ...
  it 'does not deliver the notification if the app runner is still processing the previous batch' do
    Rapns::Daemon::AppRunner.should_not_receive(:deliver)
    Rapns::Daemon::Feeder.deliver_notifications
  end

  it "delivers an undelivered notification without deliver_after set" do
    notification.update_attributes!(:delivered => false, :deliver_after => nil)
    Rapns::Daemon::AppRunner.should_receive(:deliver).with(notification)
    Rapns::Daemon::Feeder.deliver_notifications
  end

  it "delivers a notification with a deliver_after time in the past" do
    notification.update_attributes!(:delivered => false, :deliver_after => 1.hour.ago)
    Rapns::Daemon::AppRunner.should_receive(:deliver).with(notification)
    Rapns::Daemon::Feeder.deliver_notifications
  end

  it "does not deliver a notification with a deliver_after time in the future" do
    notification.update_attributes!(:delivered => false, :deliver_after => 1.hour.from_now)
    Rapns::Daemon::AppRunner.should_not_receive(:deliver)
    Rapns::Daemon::Feeder.deliver_notifications
  end

  it "does not deliver a previously delivered notification" do
    notification.update_attributes!(:delivered => true, :delivered_at => Time.now)
    Rapns::Daemon::AppRunner.should_not_receive(:deliver)
    Rapns::Daemon::Feeder.deliver_notifications
  end

  it "does not deliver a notification that has previously failed delivery" do
  ...
  it "delivers notifications when started" do
    Rapns::Daemon::Feeder.should_receive(:deliver_notifications).at_least(:once)
