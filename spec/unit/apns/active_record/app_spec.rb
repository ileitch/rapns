...
describe Rapns::ActiveRecord::App do
  it 'does not validate an app with an invalid certificate' do
    app = Rapns::Apns::ActiveRecord::App.new(:key => 'test', :environment => 'development', :certificate => 'foo')
    app.valid?
  ...
  it 'validates a certificate without a password' do
    app = Rapns::Apns::ActiveRecord::App.new :key => 'test', :environment => 'development', :certificate => TEST_CERT
    app.valid?
    app.errors[:certificate].should == []
  end

  it 'validates a certificate with a password' do
    app = Rapns::Apns::ActiveRecord::App.new :key => 'test', :environment => 'development',
