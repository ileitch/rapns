...
describe Rapns::Daemon::Apns::Delivery do
  ...
  describe "when delivery fails" do
    ...
    it "sets the notification as failed" do
      backend.should_receive(:mark_failed).with(notification, 4, "Missing payload")
      perform
    end
    ...
    context "when the APNs disconnects without returning an error" do
      ...
      it 'sets the error description on the notification' do
        backend.should_receive(:mark_failed).with(notification, nil, "APNs disconnected without returning an error.")
        perform
      end
    end
  end
